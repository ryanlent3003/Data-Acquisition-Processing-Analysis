...
from supporting_scripts import NSIDC_Data
import netrc
import base64
import getpass
...
class ASODataTool:
    ...
    def cmr_search(self, time_start, time_end, region, bounding_box):
        ...
        try:
            ...
            # Remove items that contain '.xml'
            #self.url_list = [url for url in self.url_list if '.xml' not in url]
    ...
    def cmr_download(self, dpath, region):
        ...
        for i in tqdm_notebook(range(len(self.url_list))):
            NSIDC_Data.cmr_download(self.url_list[i],credentials, dpath,region, False)
    ...
    def get_bounding_box(region):       
        bounding_box = list(region.total_bounds)
...
class ASODataProcessing:
    ...
    def convert_tiff_to_parquet_multiprocess(self, input_folder, output_res, region):
        ...
        dir = f"files/ASO/"
        folder_path = os.path.join(dir, f"{region}/{input_folder}")
        print(folder_path)
